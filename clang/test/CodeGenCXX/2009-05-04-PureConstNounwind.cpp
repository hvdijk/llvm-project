...
// CHECK: attributes [[TF]] = { {{.*}} }
// CHECK: attributes [[RN]] = { nosideeffects willreturn memory(none){{.*}} }
// CHECK: attributes [[RO]] = { nosideeffects willreturn memory(read){{.*}} }
// CHECK: attributes [[TF2]] = { {{.*}} }
// CHECK: attributes [[RN_CALL]] = { nosideeffects willreturn memory(none) }
// CHECK: attributes [[RO_CALL]] = { nosideeffects willreturn memory(read) }
