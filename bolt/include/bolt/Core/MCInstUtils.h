...
public:
  ...
  /// Constructs a reference to the instruction referred to by the iterator.
  MCInstReference(BinaryFunction::instr_const_iterator It) : It(It) {}
  ...
  bool operator==(const MCInstReference &Other) const { return It == Other.It; }
  ...
  operator const MCInst &() const {
    assert(!empty() && "Empty reference");
    return getMCInst();
  }

  bool empty() const { return It.getFunction() == nullptr; }
...
private:
  BinaryFunction::instr_const_iterator It;
};
